...
export default function Home() {
  ...
  return (
    <div className="relative h-screen w-full bg-gray-300">
      ...
      <motion.div initial={{width:120 }} animate={{ width: show ? 120 : 320 }} transition={{ duration:0.5 }} className="absolute top-0 left-0 w-full h-full bg-whie">
          ...
          <motion.div animate={{ opacity: show ? 0 : 1 }} transition={{ duration:0.5 }} className="text-xl" onClick={() => { setShow(!show) }}>
            <h3 className="text-3xl font-bold pt-12">MENU</h3>
            {
              items.map((item, index) => {
                return (
                  <motion.div key={index} className="py-4 text-lef w-[220px] bg-gray-100 p-3 m-2 rounded-md shadow-md" initial={{ opacity:0, x:10 }} animate={{ opacity: show ? 0 : 1,x: show ? -10 : 10}} transition={{ duration:0.2, delay:index * 0.1 }}>
                    <p>{item.title}</p>
