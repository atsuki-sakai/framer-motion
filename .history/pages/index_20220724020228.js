          ...
          <motion.div animate={{ opacity: show ? 0 : 1 }} transition={{ duration:0.5 }} className="text-xl" onClick={() => { setShow(!show) }}>
            <h3 className="text-3xl font-bold pt-12">MENU</h3>
            {
              items.map((item, index) => {
                return (
                  <motion.div key={index} className="py-4 text-lef w-[100px] bg-green-500" initial={{ opacity:0, x:10 }} animate={{ opacity: show ? 0 : 1,x: show ? -10 : 10}} transition={{ duration:0.2, delay:index * 0.1 }}>
                    <p>{item.title}</p>
